...
from mas.memory import MASMemoryBase, GMemory, MemoryBankGraphMASMemory, SelectiveMemMASMemory
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Run dual MedMCQA scenario with shared global memory.')
    ...
    base_dir = os.path.join('./.db', model_type, 'medmcqa_dual', args.mas_type, 'memory', args.mas_memory)
    global_dir = os.path.join(base_dir, 'global')
    phys_dir = os.path.join(base_dir, 'physiology')
    pharma_dir = os.path.join(base_dir, 'pharmacology')
    log_dir = os.path.join('./logs', 'medmcqa_dual', args.mas_type, 'memory', args.mas_memory, model_type)
    os.makedirs(global_dir, exist_ok=True)
    os.makedirs(phys_dir, exist_ok=True)
    os.makedirs(pharma_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)

    phys_manager = build_task_manager('medmcqa_physio_30', args.mas_type, args.mas_memory, args.max_trials, phys_dir, log_dir)
    pharma_manager = build_task_manager('medmcqa_pharma_30', args.mas_type, args.mas_memory, args.max_trials, pharma_dir, log_dir)

    phys_manager.mem_config.update(hop=1)
    pharma_manager.mem_config.update(hop=1)

    phys_mem = build_mas(phys_manager, args.reasoning, args.mas_memory, args.model)
    pharma_mem = build_mas(pharma_manager, args.reasoning, args.mas_memory, args.model)

    # Shared global memory retriever (only for supported memory types)
    global_mem = None
    if args.mas_memory == "g-memory":
        ...
        global_mem = SelectiveMemMASMemory(
            ...
            global_config={
                'working_dir': global_dir,
                'task_name': 'medmcqa_dual_global',
            },
            llm_model=GPTChat(model_name=args.model),
            embedding_func=EmbeddingFunc(CONFIG.get('embedding_model', "sentence-transformers/all-MiniLM-L6-v2"))
        )
    elif args.mas_memory == "memgraph":
        global_mem = MemoryBankGraphMASMemory(
            namespace='memgraph',
            global_config={
                'working_dir': global_dir
            },
    ...
    if global_mem is not None:
        set_global = getattr(phys_mem, "set_global_retriever", None)
        if callable(set_global):
            phys_mem.set_global_retriever(global_mem)
        set_global = getattr(pharma_mem, "set_global_retriever", None)
        if callable(set_global):
            pharma_mem.set_global_retriever(global_mem)

    total = min(len(phys_manager.tasks), len(pharma_manager.tasks))
    ...
    for start in range(0, total, batch_size):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            fut_phys = executor.submit(run_batch, phys_manager, start, end)
            fut_pharma = executor.submit(run_batch, pharma_manager, start, end)
            phys_messages = fut_phys.result()
            pharma_messages = fut_pharma.result()
        ...
        if global_mem is not None:
            ...
            for msg in phys_messages:
                if msg is None:
                    continue
                if isinstance(global_mem, GMemory):
                    global_mem.add_memory_with_source(msg, source_id="Physiology", raw=True)
                elif isinstance(global_mem, MemoryBankGraphMASMemory):
                    global_mem.add_memory_from_peer(msg, source_id="Physiology")
            for msg in pharma_messages:
                if msg is None:
                    continue
                if isinstance(global_mem, GMemory):
                    global_mem.add_memory_with_source(msg, source_id="Pharmacology", raw=True)
                elif isinstance(global_mem, MemoryBankGraphMASMemory):
                    global_mem.add_memory_from_peer(msg, source_id="Pharmacology")

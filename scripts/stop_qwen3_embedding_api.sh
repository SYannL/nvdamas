...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -z "${PID}" ]]; then
    echo "qwen3 embedding api pid file not found and no ${SERVED_MODEL_NAME} vLLM process found: ${PID_FILE}"
    exit 0
  fi
  echo "qwen3 embedding api pid file not found; found ${SERVED_MODEL_NAME} pid ${PID} from /proc"
fi

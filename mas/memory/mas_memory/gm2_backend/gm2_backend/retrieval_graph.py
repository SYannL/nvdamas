...
def _artifact_bundle(
    ...
    *,
    ...
    bundle.routing_decisions = {
        "fact": "local" if bundle.fact_items else "none",
        "relation": "mixed" if bundle.relation_items and global_pool else ("local" if bundle.relation_items else "none"),
        "plan": "mixed" if bundle.plan_items and global_pool else ("local" if bundle.plan_items else "none"),
        "workflow": "mixed" if bundle.workflow_items and global_pool else ("local" if bundle.workflow_items else "none"),
        "precondition": "mixed" if bundle.precondition_items and global_pool else ("local" if bundle.precondition_items else "none"),
        "repair": "mixed" if bundle.repair_items and global_pool else ("local" if bundle.repair_items else "none"),
        "closure": "mixed" if bundle.closure_items and global_pool else ("local" if bundle.closure_items else "none"),
        "blocked": "mixed" if any(item.pattern_kind == RuleType.BLOCKED.value for item in combined) else "none",
    }
    bundle.routing_weights = {
        slot: {
            "local": 1.0 if decision != "none" else 0.0,
            "global": 0.35 if decision == "mixed" else 0.0,
            "none": 0.0 if decision != "none" else 1.0,
        }
    ...
    bundle.suggested_actions = _filter_suggested_actions(
        query,
        [
        action for action, _ in sorted(suggested_votes.items(), key=lambda item: item[1], reverse=True)[:8]
        if action not in blocked_votes
        ],
    )
    warnings: list[str] = []
